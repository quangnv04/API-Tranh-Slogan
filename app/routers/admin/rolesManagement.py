...
@router.get("/admin/roles-management", response_class=HTMLResponse)
async def roomsManagement(request: Request):
    time_debug = "0.1.57"
    if DEBUG == 'True':
        time_debug = time.time()
    return templates.TemplateResponse("/admin/roles-management.html", {"request": request, "time": time_debug})
