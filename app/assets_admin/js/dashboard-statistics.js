    ...
    function getEndpoint(type) {
        return type === 'count' ? '/api/admin/statistics/order_count' : '/api/admin/statistics/revenue';
    }
    // Hàm gọi API dữ liệu
    ...
    function drawChart(data, dataType) {
        ...
        const timeField = Object.keys(data[0]).find(k => k.toLowerCase().includes('order'));
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $(".doanh_thu_hom_nay").empty().append(`
                    <p class="text-sm mb-0 text-uppercase font-weight-bold ">Doanh thu ngày hôm nay</p>
                    <h5 class="font-weight-bolder">
                        ${response.today_revenue.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}
                    </h5>
                   <p class="mb-0 ">
                    <span class="${percentRevenue.className} text-sm font-weight-bolder ">${percentRevenue.text}</span>
                    since yesterday
                    </p>
                `);

                $(".so_don_hang_hom_nay").empty().append(`
                    <p class="text-sm mb-0 text-uppercase font-weight-bold ">Số đơn hàng hôm nay</p>
                    <h5 class="font-weight-bolder">
                        ${response.today_order_count}
                    </h5>
                     <p class="mb-0">
