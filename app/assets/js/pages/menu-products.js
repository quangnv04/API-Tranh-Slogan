...
$(document).ready(function () {
    ...
    function fetchProductsByType(type, pageNum = page) {
        canFetch = false
        $.ajax({
            url: `/api/products?type=${encodeURIComponent(type)}&page=${pageNum}&limit=${limit}`,
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                ...
                canFetch = data.products.length === limit;
            },
    ...
    function createPagination(totalPages, currentPage) {
        ...
        paginationContainer.on('click', 'a.page-link', function (event) {
            ...
            if (!isNaN(newPage) && newPage > 0 && newPage <= totalPages && newPage !== currentPage) {
                window.scrollTo(0, 0);
                if (selectedType) {
                    fetchProductsByType(selectedType, newPage);
                } else {
